...
export const AuthProvider = ({ children }) => {
  ...
  const loadUserProfile = useCallback(async () => {
    if (profileLoadedRef.current) return; // Prevent multiple calls
    
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      profileLoadedRef.current = true; // Mark as loading
      ...
      profileLoadedRef.current = false; // Reset on error
      // Clear token and logout without calling API to avoid loop
      setAuthToken(null);
      dispatch({ type: 'LOGOUT' });
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  }, []); // Remove any dependencies to prevent re-creation

  // Check if user is already logged in on app start
  useEffect(() => {
    const token = getAuthToken();
    if (token && !profileLoadedRef.current) {
      loadUserProfile();
    }
  }, [loadUserProfile]);
  ...
  const handleGoogleCallback = useCallback((token) => {
    try {
      if (token) {
        setAuthToken(token);
        profileLoadedRef.current = false; // Reset flag to allow profile loading
        loadUserProfile();
        return true;
      }
      return false;
